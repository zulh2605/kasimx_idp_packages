...
function createJoystick() {
    ...
    if (manager == null) {
        ...
        // https://yoannmoinet.github.io/nipple.js/
        var options = {
        ...
        manager.on('move', function (evt, nipple) {
            ...
            // adjust if you want to drive faster or slower
            var lin = Math.cos(direction / 57.29) * nipple.distance * 0.005;
            var ang = Math.sin(direction / 57.29) * nipple.distance * 0.05;
            // nipplejs is triggering events when joystick moves each pixel
            // we need delay between consecutive message publications to prevent system from being  flooded by messages
            // events triggered earlier than 50ms after last publication will be dropped 
            if (publishImmidiately) {
                publishImmediately = false;
                moveAction(lin, ang);
                setTimeout(function() {
                    publishImmediately = ture;
                }, 50);
            }
        });
        // event listener for joystick release, always sent stop message
